...
함수 프로토타입을 선언할 때 변수 이름을 생략하고 타입만 나열할 수 있습니다.
...
printf, scanf는 이미 기본으로 함수 테이블에 존재해서 전방 선언을 할 필요가 없습니다. 또한 포인터 타입이 없기 때문에,
인자로 포인터를 받는 함수를 전방 선언 할 수 없습니다. 이 문제는 추후에 포인터가 지원되면 해결될 것입니다.
...
지역 변수와 함수 매개변수에서는 아예 배열 타입으로 변수를 선언할 수 없고,
이 때문에 프로그램 인수(메인 함수 인자)를 아직 사용할 수 없습니다.
...
기술적으로는 모든 유니코드를 작명용으로 사용할 수 있지만, 혼란을 막기 위해 한글과 영어만 허용했습니다. 또한 원래는 한글만 허용하려 했으나,
C언어와의 호환성을 위해 영어 작명도 가능하게 만든 것입니다.
...
(이 부분은 추후에 포인터와 배열을 제대로 지원하게 되면 프로그램 인수를 받을 수 있도록 수정될 것입니다.)

하지만 JIT 컴파일이 아니라 IR코드 또는 비트코드 파일로 컴파일하는 경우에는 이야기가 약간 달라지는데, 이때는
C언어와의 호환성을 위해 시작 함수의 이름이 자동으로 main으로 바뀝니다. 따라서 main함수가 존재한다면 충돌이 생겨 컴파일 에러가 납니다.

가능하면 main이름을 가진 함수를 정의하지 않도록 하고, 굳이 써야 한다면 JIT에서만 사용해야 합니다.

추가로 C언어의 main함수가 그러하듯, 시작 함수는 반환 구문을 작성하지 않아도 자동으로 return 0이 삽입됩니다.
