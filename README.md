...
지역 변수를 생성하려고 했다가 우연히 전역 변수를 수정하는 실수를 저지를 수도 있게 되었지만, 똑같은 이름의 변수가 여러개 있는 것 보단 낫다고 생각하고, 확실히 문법이 간단해졌기 때문에 만족합니다. 혹시 더 좋은 아이디어가 있다면 언제든지 문법을 수정할 의향이 있습니다.

"ㄱ: 실수" 또는 "ㄱ: 실수 = 10" 처럼 타입을 명시하는 변수 생성 방식은 대입이 아니라 생성으로 명확히 판단할 수 있지만, 위의 이유로 막아두었습니다.
